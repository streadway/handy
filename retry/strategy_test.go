...
func TestComposeOverStatusCode(t *testing.T) {
	...
	retry, err := RetryBy(Over(400), Max(2), Timeout(10*time.Second))(Attempt{
		Start:    time.Now(),
		Count:    1,
		Response: response,
	})
...
func TestComposeTimeout(t *testing.T) {
	retry, err := RetryBy(Max(2), Timeout(10*time.Second))(Attempt{
...
func TestComposeErrors(t *testing.T) {
	...
	retry, err := RetryBy(Max(2), Timeout(10*time.Second), Errors())(Attempt{
		Start: time.Now(),
		Count: 1,
		Err:   innerErr,
	})
...
func TestComposeEOF(t *testing.T) {
	retry, err := RetryBy(Max(2), Timeout(10*time.Second), EOF())(Attempt{
		Start: time.Now(),
		Count: 1,
		Err:   io.EOF,
	})
